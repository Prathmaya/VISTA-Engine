...
try:
    myappid = 'mycompany.exotransit.engine.v5.3.2'
    ...
    for module_name, pip_name in REQUIRED_PACKAGES.items():
        try:
            ...
            try:
                ...
                print(f"Deployment failure: {pip_name} -> {e}")
...
def fetch_nasa_planetary_intel(target_name):
    ...
    if HAS_ASTROQUERY:
        try:
            ...
            print(f"TAP Query Exception Handled: {e}")
...
def run_pipeline():
    ...
    worker_thread = threading.Thread(target=_async_pipeline_worker, args=(user_input,), daemon=True)
    worker_thread.start()
...
root = ctk.CTk()
root.title("ExoTransit Engine v5.3.2")
...
ctk.CTkLabel(sidebar, text="ExoTransit Engine", font=("Segoe UI", 16, "bold"), text_color="#2980b9").pack(pady=(20, 2), padx=20, anchor="w")
ctk.CTkLabel(sidebar, text="Exo Planet Analyzer v5.3.2", font=("Segoe UI", 10), text_color="#7f8c8d").pack(pady=(0, 15), padx=20, anchor="w")
...
metrics_label = ctk.CTkLabel(metrics_frame, text="Orbit Period   : -- Days\nTransit Depth  : -- ppt\nPlanet Radius  : -- x Earth Rad\nDiscovery Mode : Standby\nClassification : Standby", font=("Consolas", 11), justify="left", wraplength=220)
...
launch_btn.pack(pady=5, padx=20, anchor="w")
...
export_btn.pack(pady=5, padx=20, anchor="w")
